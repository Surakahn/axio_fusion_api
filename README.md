...
The current 2026-07-23 v9 serving cohort was rediscovered from the two
configured channels and fully reranked before admission. It contains 131
physical/logical candidates, all with a completed research-prior record. Three
strict streaming samples admitted 34 profiles; 11 were rejected by the
90-second ceiling and 86 by stream/protocol stability. Every admitted profile
has exactly three successful strict-stream samples, and the ready handoff has
complete `primary_solver`, `judge`, and `synthesizer` role coverage. This is an
...
For the v9 serving cohort, the calibration retained all 34 text-admitted
profiles and marked 18 as operationally tool-call eligible. The remaining tool
probe outcomes are retained as safe status counts only and do not revise text
stream admission, research ranking, or any benchmark policy.
...
run is `private/prefusion_full_live_20260722.capability_axes.v6.registry.private.json`.
It contains 34 strictly stream-verified physical profiles and 34 logical model
entries for runtime loading. The associated report and safe receipt use the
same `capability_axes.v6` prefix. The v6 research prompt contract performs
candidate-scoped fact extraction before capability-axis and role mapping; it
maps documented structured output, tool calling, verification, and named task
families without treating public benchmark scores as Axio benchmark results.
Earlier v2, v5, and operational-v1 artifacts remain diagnostic history and
must not replace the v6 registry.
