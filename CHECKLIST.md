...
- [x] 复制、I/O 或目标替换失败时清理临时文件，并保留原有可恢复目标副本。
- [x] 覆盖成功发布、替换失败、旧副本保留、临时文件清理和备份重开；账本专项 `23 passed`，
  相关预算/部署/真实增量流专项 `41 passed`。
- [x] 全量 Python 3.11 回归 `1179 passed`；提交 `b128137` 已推送并以 `setsid/nohup` 受控
  发布至 Axio 18900 PID `3355306`。health、registry、runtime routing、三档 route-plan 和
  8317 CPA Plus 验证通过；仅保留 `pre-b128137` 回滚副本。
- [ ] 仍需在隔离卷完成真实磁盘满、只读和进程崩溃恢复演练；本实现不等同于跨主机账本。
...
- [x] 公网部署合同在每日预算 + `shared_required` 时要求 ledger path；跨实例并发、幂等、
  ...
  至 Axio 18900（PID `2960979`）；恢复语义提交 `132daa9`、备份/锁故障提交 `95c5b3b`
  已推送并受控发布至 PID `3025376`；本轮全量回归 `1169 passed`。
