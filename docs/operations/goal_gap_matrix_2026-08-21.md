...
| 商业级运维 | **partial** | 生产 health ready；proxy auto；setsid 服务；atomic/safe receipts；secret/raw output 隔离 | 当前服务 auth 未启用；pricing/context/tool 能力字段为 unknown；跨 provider diversity 不足 | 按部署策略决定 auth；baseline 后补齐 admission metadata 和运行时告警 |
