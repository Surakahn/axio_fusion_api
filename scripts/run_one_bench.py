#!/usr/bin/env python3
"""Run one benchmark for one model and save result."""
import json, os, time, re, urllib.request, sys
from pathlib import Path

if len(sys.argv) < 4:
    print("Usage: run_one_bench.py <model> <bench_file> <bench_label> <task_type> [out_dir]")
    sys.exit(1)
...
for i, case in enumerate(cases):
    ...
    t1 = time.time()
    try:
        resp = call_axio(model, messages, max_tok)
...
result = {
    "model": model, "benchmark": bench_label, "task_type": task_type,
    "accuracy": round(acc, 3), "correct": sum(scores), "total": len(scores),
    "avg_latency_s": round(avg_lat, 1), "errors": errs, "wall_s": round(wall, 1),
}

Path(out_dir).mkdir(parents=True, exist_ok=True)
out_file = os.path.join(out_dir, f"{model}_{bench_label.replace(' ','_')}.json")
with open(out_file, 'w') as f:
    json.dump(result, f, indent=2)

print(f"[{bench_label}] {model}: {sum(scores):.0f}/{len(scores)} ({acc:.0%}) {avg_lat:.1f}s | Saved: {out_file}", flush=True)
