...
def build_parser() -> argparse.ArgumentParser:
    ...
    enrollment.add_argument(
        "--focus-manifest",
        default=None,
        help="Non-secret focus manifest; required model refs are preserved through all probe limits.",
    )
    enrollment.add_argument(
        "--restrict-to-required-models",
        action="store_true",
        default=False,
        help="Restrict the enrollment probe to only the models declared in the focus manifest.",
    )
...
def cmd_enroll_providers(args: argparse.Namespace) -> int:
    ...
    payload = enroll_provider_channels(
        ...
        restrict_to_required_models=bool(args.restrict_to_required_models),
    )
    _emit_json(payload)
    return 0 if payload.get("status") == "ready" else 2
